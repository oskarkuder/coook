...
Rules:
- Reconstruct the full recipe: every ingredient with a numeric amount and unit, and every step in order.
...
export async function structureRecipe(input: {
  ...
  return normalise(JSON.parse(content) as StructuredRecipe);
}

/** Defends the UI against a model that returns something slightly off-spec. */
function normalise(recipe: StructuredRecipe): StructuredRecipe {
  const servings =
